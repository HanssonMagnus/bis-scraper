...
from bis_scraper.utils.file_utils import create_directory, find_existing_files
...
class PdfConverter:
    ...
    def __init__(
        ...
        self.institutions = [normalize_institution_name(i) for i in institutions] if institutions else None
    ...
    def convert_institution(self, institution: str) -> None:
        ...
        if self.institutions is not None and normalized_institution not in self.institutions:
        ...
        if self.limit is not None and self.limit > 0:
            logger.info(f"Limiting conversion to {self.limit} files for {normalized_institution}")
    ...
    def _convert_pdf(self, pdf_path: Path, output_dir: Path, institution: str) -> None:
        ...
        try:
            ...
            if txt_path.exists() and not self.force_convert:
                ...
                skip_message = f"Skipping {file_code} (already converted to {txt_filename})"
